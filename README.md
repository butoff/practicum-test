...
случайный человек из интернета или даже сам автор) допустит ошибку в параметре:
...
предназначенный для создания строк класс StringBuilder (https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/-string-builder/)
...
А лучше пойти еще дальше и применить отсутствующую в Java интерполяцию строк:
...
запятая, а нам нужен перевод строки)
